...
public interface NoteMapper extends tk.mybatis.mapper.common.Mapper<Note> {
	...
	// 查询所有
	@Select("select id,note_name,note_remark,note_time from note order by note_time  DESC")
	List<Note> queryAll();

	// 插入
	@Insert(" insert into note (note_name,note_content,note_remark,note_time) values(#{noteName}, #{noteContent},#{noteRemark},#{noteTime}) ")
	...
	// 更新
	@Update("update note set  note_name = #{noteName},note_content = #{noteContent},note_remark = #{noteRemark},note_time = #{noteTime}  where id = #{id}")
